    ...
    var viewport = new Ext.Viewport({
      ...
      items:[
        ...
        new Ext.Panel({
          ...
          split:false,
          title: '&copy; Inform&aacute;tica Gesfor',
          collapsible: false,
          animate: false
        })
      ]
...
        {text : 'light red',
 ...
    });
  ...
  generateMashupScript : function()
  {
    ...
    //Head
    var edtContainer = this.editorsPanel.divs[this.editorsPanel.HEADER];
    if(edtContainer.isVisible() && edtContainer.containsMashup()) {
      mashupId = edtContainer.getMashupId();
      script.push('  '+_this.getMashupBuilderJS(mashupId, edtContainer.getName()));
    }
    //Layout
  ...
  composePage: function(preview)
  {
    var divs   = this.generateDivs();
    var script = this.generateMashupScript();

    var buf = [];
    buf.push('<html>');
    buf.push('<head>');
    buf.push('<title>MyCocktail Page Editor - HTML Example</title>');
    buf.push('<link type="text/css" rel="stylesheet" href="'+afrous.baseURL+'/css/themes.css" />');
  ...
  preview : function(content)
  {
    ...
    buf.push('<form action="'+romulus.pageEditor.config.PAGE_PREVIEW_URL+'" method="POST" id="myform">');
    buf.push('<textarea style="display:none" name="page">'+ this.composePage() + '</textarea>');
    buf.push('</form>');
    buf.push('</body>');
    buf.push('</html>');

    var msgWindow = window.open(afrous.config.EMPTY_URL,"","width=1050,height=600,scrollbars=yes,centerscreen=yes,menubar=no,location=no");
  ...
  loadJson: function(json)
  {
    ...
    for(var i=0; i<json['divs'].length; i++)
    {
      ...
      if(edtContainer != null)
      {
        ...
        {
          edtContainer.addMashup(divJson['content'], divJson['title']);
        }
        else
        {
  ...
  toConfig: function()
  {
    ...
    for(var i=1; i<= this.editorsPanel.NUMBER_OF_EDITORS; i++)
    {
      ...
      if(edtContainer.isVisible())
      {
        if(edtContainer.containsMashup())
        {
          divs.push({name: edtContainer.getDivName(), 
                     typeOfContent: 'mashup',
                     content: edtContainer.getMashupId(),
                     title: edtContainer.getMashupTitle(),
                     css: {}});
        }
        else
        {
          divs.push({name: edtContainer.getDivName(),
                     typeOfContent: 'text',
                     content: edtContainer.getTextFromEditor(),
                     css: {}});
        }
      }
    }
  ...
  newPage : function() {
    ...
    this.setPageKeyID(null);
  }
  ,

  setPageKeyID : function(key) {
    this.pageKeyID = key;
    key = key || '';
...
//    }
  ...
  loadProcessFromServer : function(key) {
    this.newPage();
    this.setPageKeyID(key);
    afrous.lang.loadScript(romulus.pageEditor.config.PAGE_REGISTRY_URL + '?id=' + key);
  }
  ,
...
//    this.configs[key] = conf;
  ...
  openProcessFromServer : function() {
    if (!this.openMashupDialog) {
      this.openMashupDialog = new OpenProcessDialog();
      this.openMashupDialog.setCallback(function(mashupId) {
        if (mashupId) this.editorsPanel.addMashup(mashupId);
      }, this);
    }
    this.openMashupDialog.show();
  }
  ,
      ...
      if(group){
        group.up('div').toggleClass('collapsed');
          ...
          if(t && !e.getTarget('a', 2)){
              ...
              romulus.pageEditor.app.change(option);
...
Ext.extend(EditorsPanel, Ext.Panel, {
  ...
  addMashup: function(mashupId,title)
  {
    ...
    this.activeDiv.addMashup(mashupId,title);
  }
});
...
var FCKEditorContainer = function(container, divNumber, cls)
{
  ...
  this.container.createChild('<div id="'+this.name+'mshpContainer" style="float:left; width:100%; height:100%; display:none;" class="border">'+
                             ...
                             '    <img class="edit-mashup-title" src="./icons/mashupbuilder/accept.png" alt="Accept" id="'+this.name+'SetEditTitleMashup" style="display:none;"/>'+
...
Ext.extend(FCKEditorContainer, Ext.Resizable, {
  ...
  addMashup: function(mashupId,title)
  {
    ...
    if(title) {
        this.mashupTitle = title;
        $("#"+this.name+'TitleMashup').html(title);
    }
  }
  ,
...
romulus.pageEditor.config = {
  PAGE_SAVE_URL     : afrous.baseURL+'/page/savePage',
  PAGE_PRIVATE_URL  : afrous.baseURL+'/page/loadPages',
  PAGE_REGISTRY_URL : afrous.baseURL+'/page/showPage',
  PAGE_PREVIEW_URL  : afrous.baseURL+'/page/preview'
};

$(document).ready(function ()
{
  romulus.pageEditor.app = new romulus.pageEditor.RomulusPageEditor($("#divContainer"));
  romulus.pageEditor.app.initialize();
});
