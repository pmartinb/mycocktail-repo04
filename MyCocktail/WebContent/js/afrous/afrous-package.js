 ...
 *
 * This program is a modification of Afrous JavaScript - Standard Packages
 * which  is freely distributable under the terms of an MIT-style license.
 * Afrous JavaScript - Editor: 2007 - 2008 Shinichi Tomita (stomita@mashmatrix.com)
 */
...
basic.register(new afrous.UnitAction({
  type : 'Comparator',
  label : 'Comparator',
  description : 'Compare the values of an object',
  inputs : [
    {name : 'mode',
  ...
  execute : function(request, callback) {
    ...
    var conditions = [];
    for (var i=0; i<10; i++) {
      var value1 = request.params['property'+i];
      var operand = request.params['operand'+i];
      var value2 = request.params['value'+i];
      if ((typeof value1 != 'undefined' &&
           typeof operand != 'undefined' &&
           typeof value2 != 'undefined') ||
          (typeof value1 != 'undefined' &&
           (operand == 'is empty' || operand == 'is null'))) {
        conditions.push({value1 : value1, operand : operand, value2 : value2});
      }
    }

    var flag = true;
    for(var i=0; i<conditions.length; i++) {
      flag = checkCondition(conditions[i])
      if (flag) {
        if (mode=='OR') {
          flag = true;
          break;
    ...
    function checkCondition(condition) {
      switch (condition.operand) {
        ...
        case '!=' :
          return condition.value1 != condition.value2;
        case 'includes' :
          var pstr = afrous.lang.toString(condition.value1);
          return pstr.indexOf(condition.value2)>=0;
        case 'not includes' :
          var pstr = afrous.lang.toString(condition.value1);
          return pstr.indexOf(condition.value2)<0;
        case 'matches' :
          var pstr = afrous.lang.toString(condition.value1);
          var regexp = new RegExp(condition.value2);
          return regexp.test(pstr);
        case 'is empty' :
          return condition.value1 && condition.value1.length == 0;
        case 'is null' :
          return condition.value1 == null;
        default :
          return true;
      }
    }
...
basic.register(new afrous.UnitAction({
  ...
  execute : function(request, callback) {
    ...
    if(masterKey) {
      request.readParam(masterKey, {
        onSuccess : function() {callback.onSuccess(request.params[masterKey])},
        onFailure : callback.onFailure
      });
    }
  }
...
array.register(new afrous.UnitAction({
      ...
      afrous.async.iterate(
        ...
        function(a, i, arr, cb) {
          ...
          for (var key in params) {
            if(key != 'array') {
              childProc.setParam(key, params[key]);
            }
          }
          childProc.start(cb);
        }, 
  ...
  prepareInnerParams : function(request, callback) {
    ...
    var innerParams = arr && arr.length > 0 ? {element : arr[0], index : 0} : {};  
    for (var key in request.params) {
      if(key != 'array') {
        innerParams[key] = request.params[key];
      }
    }
    
    callback.onSuccess(innerParams);
  }
...
array.register(new afrous.UnitAction({
  type : 'Remove',
  label : 'Remove',
  description : 'Delete an element of the given array and returns the results in a new array.',
  inputs : [
    {name : 'array',
      label : 'array to operate',
      type : 'Object[]'},
    {name : 'property',
      label : 'element name to delete',
      type : 'String',
      options : []
    }
  ]
  ...
  execute : function(request, callback) {
    ...
    var rarr = request.params['array'];
    var element = request.params['property'];
    for(var i=0; i<rarr.length; i++) {
        var obj = rarr[i];
        delete obj[element];
    }
//    var rarr = request.params['array'].splice(request.params['array'].indexOf(element),1);
    callback.onSuccess(rarr);
...
array.register(new afrous.UnitAction({
  ...
  execute : function(request, callback) {
    ...
    var rarr = (request.params['array'] || []).sort(function(elem1, elem2) {
      ...
      if( val1 && (typeof val1 == "string") && (val1.match("€") || val1.match("$") || val1.match("£"))  && val1.length <16 ){
        val1 = val1.replace(/\,/g, '.');
        val1 = parseFloat(val1);   
      }
      if(  val2 && (typeof val2 == "string") && (val2.match("€") || val2.match("$") || val2.match("£")) && val2.length <16 ){
        val2 = val2.replace(/\,/g, '.');
...
array.register(new afrous.UnitAction({
  ...
  execute : function(request, callback) {
    ...
    for (var i=0,len=namesArray.length; i<len; i++) {
      var hashObjectAux = {};
      hashObjectAux['value'] = namesArray[i];
      hashObjectAux['repetitions'] = hashObject[namesArray[i]];
...
string.register(new afrous.UnitAction({
  ...
  description : 'Generate String from template. Input properties can be refered and embedded by following notation<br/>#{<i>propname</i>}',
  inputs : [
    {name : 'template',
      label : 'template',
      type : 'String',
  ...
  execute : function(request, callback) {
    ...
    var string = templateStr.replace(/#{([^}]+)}/g, function($1,$2) {
      var val;
      try {
...
string.register(new afrous.UnitAction({
  type : 'Replace',
  label : 'Replace',
  description : 'Finds a match between a regular expression and a string, and replaces the matched substring with a new substring',
  inputs : [
    {name : 'string',
      label : 'String to match',
      type : 'String'}
...
//          typeof operand != 'undefined' &&
          typeof value != 'undefined') {
        replacements.push({find : property, replace : value, flags : operand});
      }
    }

    for(var i=0; i<replacements.length; i++) {
      str = str.replace(new RegExp(replacements[i].find, getFlags(replacements[i].flags)), replacements[i].replace);
    }
...
basicRenderer.register(new afrous.RenderingUnitAction({
  ...
  render : function(params, el) {
    var templateStr = params['template'];
    params = afrous.lang.extend({}, params);
    delete params.template;
    ...
    el.appendChild(afrous.dom.parseHTML(html));
  }
...
//basic = new afrous.UnitActionPackage('Math');
...
basic.register(new afrous.UnitAction({
  ...
  execute : function(request, callback) {
      var operate = request.params['Math expression'];
      callback.onSuccess(eval(operate));
    }
