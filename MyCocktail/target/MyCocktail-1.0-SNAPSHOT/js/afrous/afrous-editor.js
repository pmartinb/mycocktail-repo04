...
//    });
  ...
  setupLayout : function() {
    ...
    this.libraryPanel = new afrous.editor.LibraryPanel(
...
        { text : 'Publish as HTML',
 ...
    publishMenu.add({
      text : 'Publish as Widget',
      cls : 'x-menu-item af-gadget-btn',
      menu: this.publishMenuGadget
    });
 ...
    });
    ...
    this.appToolbar.add({
      text : 'Page Editor',
      cls: 'x-btn-text-icon af-page-editor-btn',
 ...
    });
  ...
  createNewProcPanel : function(title, parentProcessPanel) {
    var processPanel = new afrous.editor.ProcessPanel(
  ...
  newProcess : function() {
    ...
    this.rootProcessPanel = new afrous.editor.ProcessPanel(
...
//    }
  ...
  generateJavaScript: function(onload, widget) {
    ...
    afrous.lang.forEach(afrous.lang.values(procConfig.requires), function(scriptURL) {
      ...
      } else if(scriptURL.endsWith("js/afrous/renderers/google-visualization.js") || scriptURL.endsWith("js/afrous/packages/google-data.js")) {
        buf.push('<script type="text/javascript" src="http://www.google.com/jsapi"></script>');
        if(scriptURL.endsWith("js/afrous/renderers/google-visualization.js")) {
          buf.push('<script type="text/javascript">google.load("visualization","1",{packages:["corechart","table","geomap","annotatedtimeline"]});</script>');
        }
        if(scriptURL.endsWith("js/afrous/packages/google-data.js")) {
          buf.push('<script type="text/javascript">google.load("gdata","2.x");</script>');
        }
      } else if(scriptURL.endsWith("js/afrous/renderers/telcoservices.js")) {
        buf.push('<script type="text/javascript" src="'+afrous.baseURL+'/js/jquery/jquery.js"></script>');
        buf.push('<script type="text/javascript" src="http://s.phono.com/releases/0.2/jquery.phono.js"></script>');
      }
  ...
  generateHTML: function() {
    ...
    buf.push('<html>');
    buf.push('<head>');
    buf.push('<title>MyCocktail - HTML Example</title>');
    buf.push('<link rel="shortcut icon" href="./images/mycocktail.ico" />');
    ...
    afrous.lang.forEach(afrous.lang.values(procConfig.requires), function(scriptURL) {
      if(scriptURL.endsWith("js/afrous/packages/google-data.js")) {
        buf.push('<img src="./extjs/resources/images/aero/s.gif" style="display:none" alt="" />');
      }
    });
  ...
  showTestDialog : function() { 
    ...
    buf.push('</head>');
    buf.push('<body>');
    buf.push('<form action="'+afrous.config.PROCESS_PREVIEW_URL+'" method="POST" id="myform">');
    buf.push('<textarea style="display:none" name="page">' + this.generateHTML() + '</textarea>');
    buf.push('</form>');
    buf.push('</body>');
    buf.push('</html>');
...
//  ,
  ...
  openEditor : function() {
    window.open(afrous.baseURL + '/editor.html');
  }
  ,
  ...
  setValue : function(value) {
    ...
    if (afrous.lang.isDOMNode(value)) {
      ...
      afrous.lang.forEach(afrous.lang.keys(value), function(key) {
        if (!/^(myCocktailName|myCocktailIcon)/.test(key)){
          if (afrous.lang.isSerializable(value[key])) {
            var p = this.path + (/^\w[\w\d]*$/.test(key) ? '.'+key : '["'+key+'"]');
            var cfg = {}
            if(value && value['myCocktailIcon_'+key])
              cfg.icon = value['myCocktailIcon_'+key];
            this.appendChild(new ObjectTreeNode(p, key, value[key], cfg));
          }
        }
      }, this);
...
Ext.extend(LibraryPanel, Ext.NestedLayoutPanel, {
  ...
  setupLayout : function() {
    ...
    this.toolbarPanel = new Ext.ContentPanel(this.el.createChild());
    this.toolbar = new Ext.Toolbar(this.toolbarPanel.el);
    this.loadMenu = new Ext.menu.Menu({
      items: [
      /*{ text : 'Open Public',
    ...
    this.oprPackagePanel = new Ext.ContentPanel(this.el.createChild(), { title : 'Operation' });
    ...
    this.svcPackagePanel = new Ext.ContentPanel(this.el.createChild(), { title : 'Service' });
    ...
    this.rdrPackagePanel = new Ext.ContentPanel(this.el.createChild(), { title : 'Renderer' });
    ...
    LibraryPanel.superclass.constructor.call(this, layout, { title : this.config.title });
...
var ProcessPanel = function(el, config) {
  ...
  this.paramsPanel = new afrous.editor.ParamsPanel(
    el.createChild({ cls : 'af-params' }),
    { title : 'Parameters', processPanel : this }
  );
  this.actionsPanel = new afrous.editor.ActionsPanel(
    el.createChild({ cls : 'af-actions' }),
    { title : 'Actions', processPanel : this }
  );
  this.outputPanel = new afrous.editor.OutputPanel(
...
Ext.extend(ParamsPanel, Ext.NestedLayoutPanel, {
  ...
  setParamValue : function(name, value) {
    ...
    if(node) {
      ...
      this.createNewParam(name, 'object', value, true);
    }
...
Ext.extend(ActionWidget, StackWidget, {
  ...
  renderOutputArea : function() {
    ...
    this.renderer = this.actionDef.uaction instanceof afrous.RenderingUnitAction;
...
Ext.extend(ActionInputField, Ext.util.Observable, {
  ...
  setVisible : function(visibility) {
    this.el.setVisible(visibility);
  }
  ,

  setDisplayed : function(displayed) {
    this.el.setDisplayed(displayed);
  }
  ,

  setLabel : function(label){
    var el = this.control.el.dom.parentNode.parentNode;
 ...
 * @class Iconizer
 */
var Iconizer = afrous.lang.defineClass({
  icons : { book        : "icons/mashupbuilder/book.png",
            building    : "icons/mashupbuilder/building.png",
            event       : "icons/mashupbuilder/date.png",
            calendar    : "icons/mashupbuilder/calendar.png",
            camera      : "icons/mashupbuilder/camera.png",
            cake        : "icons/mashupbuilder/cake.png",
            email       : "icons/mashupbuilder/email.png",
            fax         : "icons/mashupbuilder/printer.png",
            group       : "icons/mashupbuilder/group.png",
            house       : "icons/mashupbuilder/house.png",
            label       : "icons/mashupbuilder/tag_blue.png",
            lorry       : "icons/mashupbuilder/lorry.png",
            note        : "icons/mashupbuilder/note.png",
            phone       : "icons/mashupbuilder/phone.png",
            telephone   : "icons/mashupbuilder/telephone.png",
            user        : "icons/mashupbuilder/user_green.png",
            user_male   : "icons/mashupbuilder/user.png",
            user_female : "icons/mashupbuilder/user_female.png",
            vcard       : "icons/mashupbuilder/vcard.png",
            world       : "icons/mashupbuilder/world.png"}
  ,
  ...
  addIconToJson : function(json,iconName) {
    json.myCocktailIcon = this.icons[iconName];
  }
  ,
  
  addIconToProperty : function(json,property,iconName) {
    json['myCocktailIcon_'+property] = this.icons[iconName];
  }
  ,
...
afrous.editor.Iconizer = new Iconizer();
