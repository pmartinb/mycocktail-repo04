 ...
 *
 * This program is a modification of Afrous JavaScript - Core version 1.1
 * which  is freely distributable under the terms of an MIT-style license.
 * Afrous Core: (c) 2007 - 2008 Shinichi Tomita (stomita@mashmatrix.com)
 */
...
afrous.baseURL = (function() {
  var scripts = document.getElementsByTagName('script');
  for (var i=0; i<scripts.length; i++) {
    var m = scripts[i].src.match(/(.*)\/js\/afrous\/afrous-core\.js$/);
    if (m) {
...
afrous.lang = {
  ...
  cast : function(type, obj) {
    var isObject = /^object/.test(type);
    if (isObject && !afrous.lang.isObject(obj)) {
      obj = afrous.lang.parseJSON(obj);
    }
    ...
    arr = afrous.lang.map(arr, function(o) {
      try {
        var otype = typeof(o); 
        var ostr = afrous.lang.toString(o);
...
afrous.async = {
  ...
  sync : function(funcs, callback) {
    callback = afrous.lang.responder(callback);
...
afrous.ajax = {
  ...
  jsonp : {
    ...
    invoke : function (url, callback, options, format) {
      // Added for working with server proxy for simulating JSONP Calls 
      // with JSON and XML, instead of with JSONP directly. For supporting 
      // services which does not provide JSONP.
      if(format != null && (format == 'JSON' || format == 'json' || format == 'XML' || format == 'xml')) {
...
var ProcessDef = afrous.lang.defineClass({
  ...
  addActionDef : function(actionDef) {
    ...
    //afrous.editor.EditorApp.showRecommendations();
    this.onChange();
  }
  ,
...
//  LOGIN_URL : 'http://app.afrous.com/app/auth/info',
  ...
  PROCESS_OPEN_URL     : afrous.baseURL+'/mashup/openMashupJsonFromLocal',
  PROCESS_SAVE_URL     : afrous.baseURL+'/mashup/saveMashup',
  PROCESS_SAVE_LOC_ULR : afrous.baseURL+'/mashup/saveMashupJsonToLocal',
  PROCESS_PRIVATE_URL  : afrous.baseURL+'/mashup/loadMashups',
  PROCESS_REGISTRY_URL : afrous.baseURL+'/mashup/showMashup',
  PROCESS_PREVIEW_URL  : afrous.baseURL+'/mashup/preview',
